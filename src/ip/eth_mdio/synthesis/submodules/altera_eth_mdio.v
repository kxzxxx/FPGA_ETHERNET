...
module altera_eth_mdio #(
    parameter MDC_DIVISOR = 'd32       // MDC frequency is Avalon clock / MDC_DIVISOR. Must be <2.5MHz
) (
    input               clk,
    input               reset,
    ...
    localparam P_MDC_DIVIDE_BITS = log2ceil(MDC_DIVISOR) - 1'b1;    // Need to count to (MDC_DIVISOR/2) - 1
    ...
    localparam mdc_divide_tmp0 = MDC_DIVISOR/2;
    localparam [P_MDC_DIVIDE_BITS-1:0] mdc_divide_tmp1 = mdc_divide_tmp0[P_MDC_DIVIDE_BITS-1:0];
    ...
    always @ (posedge clk or posedge reset) begin
        if (reset) begin
            ...
            if (mdc_divide==0) begin
                mdc <= ~mdc;
                mdc_divide <= mdc_divide_tmp1 - 1'b1;
            end
            else begin
